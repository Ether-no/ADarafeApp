            ...
            <table>
                ...
                <tbody>
                    @foreach (Cart::content() as $item)
                        <tr>
                            <td class="col m3 s3 margin-cart"><a href="{{ route('detalle', $item->model->id_productos) }}"><img class="responsive-img" src="{{ asset($item->model->Foto)}}" alt=""></a></td>
                            <td class="col m3 s3 margin-cart">
                                <p>{{Str::limit($item->model->descripcion, 26)}}</p>
                            </td>
                            <td class="col m3 s3 margin-cart">
                                <div class="input-field col s12 m6">
                                    <select class="quantity" data-id="{{ $item->rowId }}"><br>
                                    ...
                                    <form action="{{ route('cart.destroy', $item->model->id_productos) }}" method="POST">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
                            ...
                            <td class="col m3 s3 margin-cart-top">$ {{ number_format($item->subtotal, 2, '.', ',') }}</td>
                        </tr>
                    @endforeach
                </tbody>
        </div>
